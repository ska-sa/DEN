...
inputs:
  ...
  gridmode:
    ...
    - type: enum
    - sybmols: [nn,kb,rect]
    doc: " Kernel and mode used for gridding: kb = Kaiser-Bessel (default with 7 pixels), nn = nearest neighbour (no kernel), rect = rectangular window."
    inputBinding:
      prefix: -gridmode
       position: 36
  gkernelsize:
  ...
  visibility-weighting-mode:
    - "null"
    - type: enum
    - sybmols: [normal,squared,unit]
    doc: "Specify visibility weighting modi. Affects how the weights (normally) stored in WEIGHT_SPECTRUM column are applied. Useful for estimating e.g. EoR power spectra errors. Normally one would use this in combination with 'no-normalize-for-weighting'."
    inputBinding:
